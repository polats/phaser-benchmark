/* eslint-disable */
// @ts-nocheck
// Official Phaser demo "Big Forest - SpriteGPU Layer Demo" by PhotonStorm
// (sandbox X4nXSf53), vendored near-verbatim and ported to run inside this
// template. A single SpriteGPULayer renders a deep, parallax-scrolling forest
// with weather, day/night tinting, clouds, rain/pollen and animated foliage —
// all on the GPU. @ts-nocheck keeps the upstream source intact without fighting
// our strict tsconfig. World size scaled down from the original; see worldWidth.
//
// Assets (public/assets/atlas/big-forest.*) are Phaser example assets.
import * as Phaser from 'phaser';
import { startBenchReadout } from './fpsReadout';

export class BigForestScene extends Phaser.Scene
{
    constructor () { super('BigForest'); }
    ...
    create ()
    {
        ...
        const worldWidth = gameWidth * 100; // original demo used 1000 (~1.4M sprites)
        ...
        const layer = this.add.spriteGPULayer('big-forest', 512 * 1024) /* original: 1024*1024*1.5 */;
        ...
        this.add.text(this.scale.width / 2, 16, layer.memberCount + ' sprites in 1 object. Drag to scroll · pinch / wheel to zoom')
            .setOrigin(0.5, 0)
            .setScrollFactor(0);

        // Drag-to-pan + pinch/wheel zoom. Works with touch (mobile) and mouse.
        // Inertia is applied in update(). Replaces the original click-edge scheme,
        // which relied on the fixed 1280 design width and broke on phone screens.
        ...
        this.input.on('pointerdown', (pointer) => {
            ...
            this.scrollSpeed = 0; // grabbing stops any glide
        });

        this.input.on('pointermove', (pointer) => {
            if (!this.dragging) return;
            // Don't pan while pinching (two fingers down) — update() handles zoom.
            if (this.input.pointer2 && this.input.pointer2.isDown) return;
            // Divide by zoom so a drag covers the same on-screen distance when zoomed in.
            const dx = (pointer.x - this.dragLastX) / this.cameras.main.zoom;
            this.dragLastX = pointer.x;
            this.scrollSpeed = -dx; // carry drag velocity into inertia
            this.cameras.main.scrollX = Math.max(0, this.cameras.main.scrollX - dx);
        });
        ...
        this.input.on('wheel', (pointer, over, dx, dy) => {
            ...
            cam.zoom = Phaser.Math.Clamp(cam.zoom * (dy > 0 ? 0.9 : 1.1), this.minZoom, this.maxZoom);
        });

        startBenchReadout(this, 'demo-forest', layer.memberCount);
    }
    ...
    update (time, delta)
    {
        ...
        {
            // Two fingers down → pinch-to-zoom; suppress panning.
            this.dragging = false;
            this.scrollSpeed = 0;
            const dist = Phaser.Math.Distance.Between(p1.x, p1.y, p2.x, p2.y);
            if (this.pinchDist > 0 && dist > 0)
            {
                cam.zoom = Phaser.Math.Clamp(cam.zoom * (dist / this.pinchDist), this.minZoom, this.maxZoom);
            }
            this.pinchDist = dist;
        }
        else
        {
            this.pinchDist = 0;
            // While not dragging, glide with inertia and decay to a stop.
            if (!this.dragging)
            {
                cam.scrollX = Math.max(0, cam.scrollX + this.scrollSpeed);
                this.scrollSpeed *= 0.9;
                if (Math.abs(this.scrollSpeed) < 0.05) this.scrollSpeed = 0;
            }
        }
